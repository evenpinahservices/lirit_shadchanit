        ...
        <div id="clients-page-root" ref={swipeRef} className="flex flex-col h-full min-h-0 gap-4 overflow-hidden relative z-0">
            {error && (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 flex items-center justify-between">
                    ...
                    <button onClick={clearError} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 text-sm font-medium">Dismiss</button>
                </div>
            )}
            ...
            <div className="flex items-center justify-between shrink-0 px-1 pt-4">
                ...
                <div className="flex items-center gap-2">
                    {isUploadInProgress ? (
                        ...
                        <Link href="/clients/new" className="inline-flex items-center justify-center rounded-md bg-red-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-red-700 transition-colors">
                            <Plus className="mr-2 h-4 w-4" />Add Client
                        </Link>
                    )}
                </div>
                        ...
                        {isLoading ? (
                            ...
                            paginatedClients.map((client, index) => (
                                <tr key={client.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{calculateAge(client.dob)} / {client.gender}</td>
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400">
                                        <div className="flex items-center gap-1"><MapPin className="h-3 w-3" />{client.location}</div>
                                    </td>
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400">
                                        <div className="flex items-center gap-1"><Briefcase className="h-3 w-3" />{client.occupationTitle || "—"}</div>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <Link href={`/clients/${client.id}?mode=edit`} id={index === 0 ? "tour-client-edit-btn" : undefined} className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" title="Edit">
                                                <Pencil className="h-4 w-4" />
                                            </Link>
                                            <Link href={`/matching?clientId=${client.id}&view=results`} className="p-2 text-gray-400 hover:text-red-600 transition-colors" title="Match">
                                                <Heart className="h-4 w-4" />
                                            </Link>
                                            <button onClick={() => handleDeleteClick(client.id)} id={index === 0 ? "tour-client-delete-btn" : undefined} className="p-2 text-gray-400 hover:text-danger-600 transition-colors" title="Delete">
                    ...
                    paginatedClients.map((client, index) => (
                        <div key={client.id} className="bg-white dark:bg-gray-950 p-4 rounded-xl shadow-sm flex items-center gap-4">
                            ...
                            <div className="flex flex-col gap-1 shrink-0">
                                <Link href={`/clients/${client.id}?mode=edit`} id={index === 0 ? "tour-client-edit-btn-mobile" : undefined} className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 bg-gray-50 dark:bg-gray-900 rounded-full">
                                    <Pencil className="h-4 w-4" />
                                </Link>
                                <Link href={`/matching?clientId=${client.id}&view=results`} className="p-2 text-red-600 hover:text-red-700 bg-red-50 dark:bg-red-900/20 rounded-full" title="Match">
                                    <Heart className="h-4 w-4" />
                                </Link>
                                <button onClick={() => handleDeleteClick(client.id)} id={index === 0 ? "tour-client-delete-btn-mobile" : undefined} className="p-2 text-danger-600 hover:text-danger-700 bg-danger-50 dark:bg-danger-900/20 rounded-full">
                                    <Trash2 className="h-4 w-4" />
                                </button>
                            </div>
                        </div>
            ...
            <div className="fixed left-0 right-0 bottom-[calc(4rem+env(safe-area-inset-bottom))] flex items-center justify-between gap-2 px-2 md:px-4 py-2 bg-white dark:bg-gray-950 z-20 md:static md:p-0 md:pb-4 md:bg-transparent shadow-[0_-2px_10px_rgba(0,0,0,0.1)] md:shadow-none overflow-visible">
                <div className="flex-shrink-0 relative z-30">
                    <ItemsPerPageSelector value={itemsPerPage} onChange={handleItemsPerPageChange} totalItems={filteredClients.length} />
                </div>
                {totalPages > 1 && (
                    <>
                        <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="flex items-center gap-1 px-2 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.875rem)' }}>
                            <ChevronLeft className="h-3 w-3 md:h-4 md:w-4" /><span className="hidden sm:inline">Prev</span>
                        </button>
                        ...
                        <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="flex items-center gap-1 px-2 py-1.5 md:px-4 md:py-2 text-xs md:text-sm font-medium text-white bg-red-600 rounded-md shadow-sm hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.875rem)' }}>
                            <span className="hidden sm:inline">Next</span><ChevronRight className="h-3 w-3 md:h-4 md:w-4" />
                        </button>
                    </>
                )}
            </div>
