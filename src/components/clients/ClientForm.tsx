...
const HEAD_COVERING_OPTIONS = ["N/A", "None", "Wig", "Tichel", "Hat", "Scarf", "Flexible"];
...
const STEPS = [
    ...
    { title: "Preferences", fields: ["ageGapPreference", "willingToRelocate", "preferredEthnicities", "preferredHashkafos", "preferredLearningStatus", "expectedHeadCovering", "preferredHeadCovering"] },
...
export function ClientForm({ client, isEditing = false, onCancel }: ClientFormProps) {
    ...
    const handleAutoPopulate = () => {
        ...
        setValue("gender", randomSelection(GENDER_OPTIONS));
        ...
        setValue("eyeColor", randomSelection(EYE_COLOR_OPTIONS));
        setValue("hairColor", randomSelection(HAIR_COLOR_OPTIONS));

        setValue("maritalStatus", randomSelection(MARITAL_STATUS_OPTIONS));
        setValue("active", true);
        setValue("children", Math.floor(Math.random() * 4));

        setValue("religiousAffiliation", randomMultiSelection(RELIGIOUS_AFFILIATION_OPTIONS));
        setValue("ethnicity", randomSelection(ETHNICITY_OPTIONS));
        setValue("tribalStatus", randomSelection(TRIBAL_STATUS_OPTIONS));
        ...
        setValue("languages", randomMultiSelection(LANGUAGES_OPTIONS));
        setValue("learningStatus", randomSelection(LEARNING_STATUS_OPTIONS));

        setValue("headCovering", randomSelection(HEAD_COVERING_OPTIONS));
        setValue("smoking", randomSelection(SMOKING_OPTIONS));
        ...
        setValue("hobbies", randomMultiSelection(["Reading", "Cooking", "Hiking", "Music", "Traveling", "Coding", "Sports"]));

        const hasMedical = Math.random() > 0.8 ? "Yes" : "No";
        setValue("medicalHistory", hasMedical);
        setValue("medicalHistoryDetails", hasMedical === "Yes" ? "Minor seasonal allergies" : "");

        setValue("medicalHistoryDetails", hasMedical === "Yes" ? "Minor seasonal allergies" : "");

        setValue("ageGapPreference", randomMultiSelection(AGE_GAP_OPTIONS));
        setValue("willingToRelocate", randomSelection(WILLING_TO_RELOCATE_OPTIONS));

        setValue("preferredEthnicities", randomMultiSelection(ETHNICITY_OPTIONS));
        setValue("preferredHashkafos", randomMultiSelection(RELIGIOUS_AFFILIATION_OPTIONS));
        setValue("preferredLearningStatus", randomMultiSelection(LEARNING_STATUS_OPTIONS));
        setValue("preferredHeadCovering", randomMultiSelection(HEAD_COVERING_OPTIONS));
        setValue("expectedHeadCovering", randomSelection(HEAD_COVERING_OPTIONS));
